...
const P = (text, o = {}) => new Paragraph({
  ...
  spacing: { before: o.before ?? 0, after: o.after ?? 60, line: o.line || 218 },
  alignment: o.align, border: o.border, shading: o.shading,
  indent: o.indent, keepNext: !!o.keepNext,
});

const NOTE = (t) => P(t, { size: 16, color: MUTED, after: 50, line: 205 });

const EYEBROW = (t) => P(t, { size: 14, bold: true, caps: true, color: MUTED, ls: 30, after: 30 });

const H1 = (t) => new Paragraph({
  children: [run(t, { size: 34, bold: true })],
  heading: HeadingLevel.HEADING_1,
  spacing: { before: 0, after: 90 },
});

const H2 = (t) => new Paragraph({
  children: [run(t, { size: 17, bold: true, caps: true, ls: 24 })],
...
const H3 = (t) => new Paragraph({
  children: [run(t, { size: 15, bold: true, caps: true, ls: 20, color: MUTED })],
...
const INVERT = (titulo, paras) => new Table({
  ...
  rows: [new TableRow({
    children: [new TableCell({
      ...
      borders: noBorders,
      children: [
        P(titulo, { size: 14, bold: true, caps: true, ls: 24, color: "C9CDCD", after: 50 }),
        ...paras.map((t, i) => P(t, { size: 16, color: PAPER, after: i === paras.length - 1 ? 0 : 60, line: 210 })),
      ],
    })],
  })],
});
...
const CALLOUT = (children) => P(children, {
  ...
  indent: { left: 100 }, before: 70, after: 70, line: 205,
});

const b = (t) => run(t, { size: 16, bold: true });
const n = (t) => run(t, { size: 16 });

/* ---------------- tabelas ---------------- */

function TABLE(headers, rows, widths, o = {}) {
  const tw = o.w || W;
  const cols = widths || headers.map(() => Math.floor(tw / headers.length));
  const cell = (txt, i, opts = {}) => new TableCell({
    width: { size: cols[i], type: WidthType.DXA },
    borders: { top: NONE, bottom: HAIR, left: NONE, right: NONE, ...(opts.borders || {}) },
    shading: opts.fill ? { type: ShadingType.CLEAR, fill: opts.fill, color: "auto" } : undefined,
    margins: { top: 36, bottom: 36, left: 0, right: 90 },
    children: (Array.isArray(txt) ? txt : [txt]).map((t, k) => P(t, {
      size: opts.size || 16, bold: opts.bold, color: opts.color,
      align: opts.align, after: 0, line: 212,
    })),
  });
  const head = new TableRow({
    tableHeader: true,
    children: headers.map((h, i) => cell(h, i, {
      size: 14, bold: true, caps: true, color: MUTED,
...
const SUBROW = (texto, span) => ({
  ...
  __: { fill: SHADE, bold: true, size: 14 },
});

// grade de campos para preencher: rótulo em cima, célula vazia embaixo
function FIELDS(labels, perRow = 4, tw = W) {
  const colW = Math.floor(tw / perRow);
  const rows = [];
  for (let i = 0; i < labels.length; i += perRow) {
    ...
    rows.push(new TableRow({
      children: slice.map((l) => new TableCell({
        ...
        borders: noBorders, margins: { top: 40, bottom: 0, left: 0, right: 90 },
        children: [P(l, { size: 13, bold: true, caps: true, color: MUTED, ls: 18, after: 0, line: 200 })],
      })),
    }));
    rows.push(new TableRow({
      children: slice.map((l) => new TableCell({
        ...
        margins: { top: 0, bottom: 66, left: 0, right: 90 },
        children: [P("", { after: 0 })],
      })),
    }));
  }
...
function CHECKS(items, perRow = 2, tw = W) {
  const colW = Math.floor(tw / perRow);
  const rows = [];
  for (let i = 0; i < items.length; i += perRow) {
    ...
    rows.push(new TableRow({
      children: slice.map((t) => new TableCell({
        ...
        margins: { top: 22, bottom: 22, left: 0, right: 90 },
        children: [P(t === null ? "" : `☐  ${t}`, { size: 16, after: 0, line: 215 })],
      })),
    }));
  }
  return new Table({
...
function TIMELINE(items, tw = W) {
  const c1 = 780, c2 = tw - c1;
  return new Table({
    width: { size: tw, type: WidthType.DXA }, columnWidths: [c1, c2], borders: noBorders,
    rows: items.map((it) => new TableRow({
      children: [
        new TableCell({
          width: { size: c1, type: WidthType.DXA },
          borders: { top: NONE, bottom: HAIR, left: NONE, right: NONE },
          margins: { top: 46, bottom: 46, left: 0, right: 80 },
          children: [P(it.h, { size: 15, bold: true, after: 0, line: 205 })],
        }),
        new TableCell({
          ...
          children: [
            P(it.t, { size: 16, bold: true, after: it.d || it.f ? 26 : 0, line: 205 }),
            ...(it.d ? [P(it.d, { size: 15, color: MUTED, after: it.f ? 26 : 0, line: 200 })] : []),
            ...(it.f ? [P(it.f.map((x) => `${x} _______`).join("   "),
              { size: 13, caps: true, bold: true, color: MUTED, ls: 14, after: 0, line: 200 })] : []),
          ],
        }),
      ],
    })),
    ...
    rows: [new TableRow({
      children: [
        new TableCell({
          width: { size: c1, type: WidthType.DXA },
          shading: { type: ShadingType.CLEAR, fill: INK, color: "auto" },
          borders: noBorders, margins: { top: 110, bottom: 110, left: 130, right: 130 },
          children: [
            P(rotulo, { size: 13, bold: true, caps: true, ls: 20, color: "C9CDCD", after: 130 }),
            P(valor, { size: 50, bold: true, color: PAPER, after: 0, line: 300 }),
          ],
        }),
        new TableCell({
          width: { size: c2, type: WidthType.DXA }, borders: noBorders,
          margins: { top: 40, bottom: 40, left: 190, right: 0 },
          children: corpo,
        }),
      ],
    })],
...
/* duas colunas lado a lado, como no caderno impresso. É o que devolve
   densidade: sem isto tudo empilha e o documento dobra de tamanho. */
const GUT = 320;
const COLW = Math.floor((W - GUT) / 2);
function COLS(esq, dir, larguras) {
  const [w1, w2] = larguras || [COLW, COLW];
  const cel = (kids, w, padRight) => new TableCell({
    width: { size: w, type: WidthType.DXA }, borders: noBorders,
    margins: { top: 0, bottom: 0, left: 0, right: padRight ? GUT : 0 },
    children: kids.length ? kids : [P("", { after: 0 })],
  });
  return new Table({
    width: { size: W, type: WidthType.DXA },
    columnWidths: [w1 + GUT, w2], borders: noBorders,
    rows: [new TableRow({ children: [cel(esq, w1, true), cel(dir, w2, false)] })],
  });
}

// faixa de identificação no topo de cada folha
function HEADER(secao) {
  return new Paragraph({
    spacing: { before: 0, after: 130 },
    border: { bottom: { style: BorderStyle.SINGLE, size: 14, color: INK } },
    tabStops: [{ type: "right", position: W }],
    children: [
      run("QT PIZZA BAR", { size: 17, bold: true, ls: 60 }),
      new TextRun({ children: ["\t"], font: FONT }),
      run(secao, { size: 13, bold: true, caps: true, ls: 26, color: MUTED }),
    ],
  });
}

const BREAK2 = () => new Paragraph({ children: [new PageBreak()] });
...
/* ---- 1. protocolo ---- */
add(
  HEADER("Folha 1 de 8 · Protocolo"),
  EYEBROW("Ensaio controlado · Hidratação do pré-fermento"),
  ...
  COLS(
    [H2("Hipótese"),
     ...
     P([b("Previsão. "), n("A mais cortante, mais forte, alvéolo grande e irregular. C mais doce, mais extensível, miolo mais regular. B entre as duas, com a melhor operação das três.")], { after: 0 })],
    [H2("Por que marinara"),
     P("Muçarela e molho pesado mascaram exatamente o que se está medindo. A pizza de prova de cada braço é marinara padronizada: 80 g de pelado triturado, 6 g de alho laminado, 0,4 g de orégano, 8 g de azeite, 0,8 g de sal. Mesma gramatura nas três, pesada na balança.", { after: 0 })],
  ),
  ...
  COLS(
    [H2("Critério de ponto"),
     ...
     CHECKS(["Cheiro de acetona ou esmalte", "Massa pegajosa, com fiapos ao puxar", "pH abaixo de 4,8"], 1, COLW)],
    [H2("Material"),
     CHECKS(["pHmetro ou fita 4,0 a 7,0", "Termômetro espeto", "Balança de 0,1 g", "3 caixas idênticas",
             "Régua", "Etiquetas", "Cronômetro", "3 potes de 200 g"], 2, COLW),
...
add(
  HEADER("Folha 2 de 8 · Farinha e fermento"),
  ...
  COLS(
    ...
    [H3("O que o W não conta"),
     ...
     FIELDS(["marca", "lote", "W do lote", "P/L", "proteína %", "malte usado"], 3, COLW)],
  ),
  ...
  COLS(
    [H3("A lógica da dose"),
     P([b("A leva a maior dose "), n("porque passa quase todas as 48 h com a levedura praticamente parada a 4 °C. A fermentação real acontece no arranque e no aquecimento, não na câmara, então o tempo longo não desconta fermento como descontaria a 18 °C.")]),
     P([b("B corta o tempo quase pela metade "), n("mas ganha 10 pontos de atividade de água, e os dois efeitos quase se cancelam. A dose cai pouco.")]),
     P([b("C cai de verdade "), n("porque a 65% a mesma população de levedura trabalha muito mais rápido e a janela de ponto fica estreita. Baixar o fermento é o que devolve largura à janela e torna o braço gerenciável.")], { after: 0 })],
    [H3("Manuseio"),
     CHECKS([
...
add(
  HEADER("Folha 3 de 8 · Cronograma mestre, revisão 2"),
  ...
  COLS(
    ...
    [H2("As duas decisões desta revisão"),
     P([b("1. Appretto igual, puntata diferente. "), n("Os horários dão 8h para B e 6h para C depois do rinfresco. O appretto é onde a bola estrutura e onde o alvéolo se define, então appretto diferente entre os braços seria confundidor. As duas fazem staglio às 17h15 e 4h45 de appretto, e as duas horas de diferença ficam na puntata, que perdoa muito mais.")]),
     P([b("2. Puntata a 22 a 24 °C, appretto a 18 °C. "), n("4h45 a 18 °C é curto para 75% de hidratação. Aquecer a puntata põe a massa em movimento antes de ela entrar na câmara e recupera o tempo sem mexer no appretto.")]),
  ...
  COLS(
    ...
    ["Passou do ponto, ou uma chegou e a outra não. ", "Passada, não tente salvar, registre a hora exata. Descompassadas, segure a pronta a 2 °C e espere: comparar em pontos diferentes é pior que atrasar o forno."],
     ].map(([t, d]) => P([b(t), n(d)], { indent: { left: 180, hanging: 180 } }))],
    [H2("Checagens"),
     TABLE(["", "B · 55%", "C · 65%"],
    [
      SUBROW("Checagem 1", 3),
...
const CW1 = 3500, CW2 = W - CW1 - GUT;   // fórmula estreita, linha do tempo larga
function ficha(a) {
  add(
    HEADER(a.header),
    CODE("Hidratação da biga", a.hid, [
      ...
      new Paragraph({
        children: [run(a.nome, { size: 30, bold: true })],
        heading: HeadingLevel.HEADING_1,
    ...
    COLS(
      ...
      [
        ...
        { c: ["Massa total", a.tot, a.tot2, "75%"], __: { bold: true, top: true, numFrom: 1 } },
         ...
         [1400, 750, 700, 650], { w: CW1 }),
       NOTE(`6 × 290 g, sobra ${a.sobra} g. Em meia escala, 3 × 290 g.`),
       H2("Aspecto alvo da biga"),
       P(a.aspecto, { size: 16, after: 0 })],
      [H2("Linha do tempo"), TIMELINE(a.tl, CW2)],
      [CW1, CW2],
    ),
    CALLOUT([b("Risco deste braço. "), n(a.risco)]),
    COLS(
      [H2("Registro objetivo"), FIELDS(REGISTRO, 2, COLW)],
      [H2("Defeitos observados"), CHECKS(DEFEITOS, 1, COLW)],
    ),
    BREAK(),
  );
}
...
ficha({
  header: "Folha 4 de 8 · Braço A · Biga Fredda",
...
ficha({
  header: "Folha 5 de 8 · Braço B · Biga Morbida",
...
ficha({
  header: "Folha 6 de 8 · Braço C · Biga Umida",
...
add(
  HEADER("Folha 7 de 8 · Forno e painel sensorial"),
  ...
  COLS(
    ...
    [H3("Na abertura, antes de enfornar"),
     ...
     FIELDS(["B · abertura", "C · abertura"], 1, COLW)],
  ),
...
/* ---- 8. consolidação ---- */
add(
  HEADER("Folha 8 de 8 · Consolidação e decisão"),
  ...
  COLS(
    [H2("Como ler o resultado"),
     ...
     P([b("Se o gradiente não aparecer, não conclua nada sobre hidratação ainda. "), n("Confira primeiro se as bigas saíram no mesmo pH. Diferença de maturação explica mais resultado estranho do que diferença de hidratação, e é o erro mais comum neste tipo de ensaio.")]),
     ...
     H2("Decisão"),
     FIELDS(["vai para o cardápio", "repete com ajuste", "descartada"], 1, COLW)],
    [H3("Próximos ensaios"),
     CHECKS([
